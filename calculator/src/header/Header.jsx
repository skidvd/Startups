...
const Header = (props) => {
    ...
    let display = [<ValueInput value={props.state.firstOperand !== undefined ? props.state.firstOperand : ''} disabled={props.state.operator || props.state.result} key='firstOperand' dispatch={props.dispatch}></ValueInput>];
    if (props.state.operator) {
        display.push(<ValueDisplay value={props.state.operator} key='operator' className="operator"></ValueDisplay>)
        display.push(<ValueInput value={props.state.secondOperand !== undefined ? props.state.secondOperand : ''} disabled={props.state.result} key='secondOperand' dispatch={props.dispatch}></ValueInput>)
